...
Browser visualization is required for a viewable target when browser automation is available. Use a localhost dev/static URL for local files; avoid `file://` unless the available browser explicitly supports this workflow. *(Nota de esta instalación: el modo live/overlay no está instalado; usa capturas de pantalla del navegador como evidencia y repórtalo como fallback signal.)*
