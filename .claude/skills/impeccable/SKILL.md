...
> **Nota de esta instalación (ver NOTICE.md):** instalación parcial transcrita del repo oficial. Referencias disponibles: `product`, `audit`, `critique`, `polish`. Los scripts son shims locales; `live`, hooks, `pin` y `palette.mjs` no están instalados. Si un comando pide una referencia ausente, descárgala de `https://raw.githubusercontent.com/pbakaus/impeccable/main/.claude/skills/impeccable/reference/<cmd>.md` antes de continuar.
...
|---|---|---|---|
| `craft [feature]` | Build | Shape, then build a feature end-to-end | reference/craft.md *(no instalada)* |
| `shape [feature]` | Build | Plan UX/UI before writing code | reference/shape.md *(no instalada)* |
| `init` | Build | Set up project context: PRODUCT.md, DESIGN.md, live config, next steps | reference/init.md *(no instalada)* |
| `document` | Build | Generate DESIGN.md from existing project code | reference/document.md *(no instalada)* |
| `extract [target]` | Build | Pull reusable tokens and components into design system | reference/extract.md *(no instalada)* |
| `critique [target]` | Evaluate | UX design review with heuristic scoring | [reference/critique.md](reference/critique.md) |
| `audit [target]` | Evaluate | Technical quality checks (a11y, perf, responsive) | [reference/audit.md](reference/audit.md) |
| `polish [target]` | Refine | Final quality pass before shipping | [reference/polish.md](reference/polish.md) |
| `bolder [target]` | Refine | Amplify safe or bland designs | reference/bolder.md *(no instalada)* |
| `quieter [target]` | Refine | Tone down aggressive or overstimulating designs | reference/quieter.md *(no instalada)* |
| `distill [target]` | Refine | Strip to essence, remove complexity | reference/distill.md *(no instalada)* |
| `harden [target]` | Refine | Production-ready: errors, i18n, edge cases | reference/harden.md *(no instalada)* |
| `onboard [target]` | Refine | Design first-run flows, empty states, activation | reference/onboard.md *(no instalada)* |
| `animate [target]` | Enhance | Add purposeful animations and motion | reference/animate.md *(no instalada)* |
| `colorize [target]` | Enhance | Add strategic color to monochromatic UIs | reference/colorize.md *(no instalada)* |
| `typeset [target]` | Enhance | Improve typography hierarchy and fonts | reference/typeset.md *(no instalada)* |
| `layout [target]` | Enhance | Fix spacing, rhythm, and visual hierarchy | reference/layout.md *(no instalada)* |
| `delight [target]` | Enhance | Add personality and memorable touches | reference/delight.md *(no instalada)* |
| `overdrive [target]` | Enhance | Push past conventional limits | reference/overdrive.md *(no instalada)* |
| `clarify [target]` | Fix | Improve UX copy, labels, and error messages | reference/clarify.md *(no instalada)* |
| `adapt [target]` | Fix | Adapt for different devices and screen sizes | reference/adapt.md *(no instalada)* |
| `optimize [target]` | Fix | Diagnose and fix UI performance | reference/optimize.md *(no instalada)* |
| `live` | Iterate | Visual variant mode *(no disponible en esta instalación)* | — |

*(Para comandos marcados "no instalada": descargar la referencia oficial según NOTICE.md antes de ejecutar el comando.)*
...
1. **No argument**: the user is asking "what should I do?" Make the menu context-aware instead of static. Setup has already run `context.mjs`; if that reported `NO_PRODUCT_MD` the project has no captured context yet, so lead the menu with `/impeccable init` as the top recommendation (one line on why) and still show the rest below; don't silently jump into init. Otherwise run `node .claude/skills/impeccable/scripts/context-signals.mjs` once and read its JSON, then lead with the **2-3 highest-value next commands**, each with a one-line reason pulled from the signals, followed by the full menu (the table above, grouped by category). **Never auto-run a command; the recommendation is a suggestion the user confirms.**
   ...
   **If `scan.targets` is non-empty and `setup.platform` is not `ios`/`android`/`adaptive`, run `node .claude/skills/impeccable/scripts/detect.mjs --json <scan.targets joined by spaces>` once** (the detector over local files; it reads HTML/CSS). Fold the hits into your picks: many quality / contrast hits → `audit` or `polish`; a specific slop family → the matching command. If detect errors or the tree is large and slow, skip it and recommend the user run `audit` themselves; never block the suggestion on it.
