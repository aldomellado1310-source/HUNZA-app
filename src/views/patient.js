...
function renderPatient(app) {
  ...
  if (p.pendingAppointment) {
    ...
    <div class="bg-blue-50 border border-blue-100 p-5 rounded-3xl relative overflow-hidden mb-2">
      ...
      <div class="relative z-10">
        ...
        <h3 class="font-serif text-lg text-stone-800 mb-4">${p.pendingAppointment}</h3>
        <a href="${waApptLink}" target="_blank" class="inline-flex items-center gap-2 bg-blue-600 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-blue-700 transition shadow-md shadow-blue-200">
  ...
  const milestones = (p.milestones || []).map((m, idx) => `
    <div class="snap-center shrink-0 w-60 bg-white p-5 rounded-3xl border ${m.completed ? 'border-stone-800 shadow-md' : 'border-stone-200 opacity-70'} relative">
      ...
      <span class="text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-1.5 block">${m.day}</span>
      <h4 class="font-serif font-semibold text-stone-800 mb-2 leading-tight">${m.title}</h4>
      <p class="text-sm text-stone-500 leading-relaxed">${m.desc}</p>
      ${idx < p.milestones.length - 1 ? `<div class="absolute top-1/2 -right-4 w-4 h-[2px] ${m.completed ? 'bg-stone-800' : 'bg-stone-200'}"></div>` : ''}
    </div>`).join('')

  const faqs = (p.faqs || []).map(faq => `
    <div class="bg-white border border-stone-200 rounded-2xl overflow-hidden">
      <button class="faq-toggle w-full px-5 py-4 text-left flex justify-between items-center bg-white hover:bg-stone-50 transition">
        <span class="font-medium text-sm text-stone-800 pr-4">${faq.q}</span>
        <i data-lucide="chevron-down" class="w-4 h-4 text-stone-400 transition-transform duration-300"></i>
      </button>
      <div class="hidden px-5 pb-4 text-sm text-stone-500 leading-relaxed border-t border-stone-100 pt-3 bg-stone-50/50">${faq.a}</div>
    </div>`).join('')

  app.innerHTML = `
  <div id="patient-view" class="min-h-screen bg-stone-50 flex flex-col pb-6">
    ...
    <main class="max-w-md mx-auto w-full flex-1 px-4 py-6 flex flex-col gap-8">
      <div>
        ...
        <h1 class="font-serif text-3xl text-stone-800 mb-4">${p.name}</h1>
      ...
      <div class="relative">
        ...
        <div class="flex overflow-x-auto gap-4 pb-4 px-1 no-scrollbar snap-x cursor-grab active:cursor-grabbing">
          ${milestones || '<p class="text-stone-400 text-sm">Sin hitos cargados aún.</p>'}
        </div>
      ...
      <div>
        ...
        <div class="space-y-2">${faqs || '<p class="text-stone-400 text-sm">Sin preguntas cargadas aún.</p>'}</div>
      </div>

      ${p.quote ? `<div class="text-center px-6 py-4 bg-stone-100 rounded-3xl mt-2">
        <p class="font-serif italic text-stone-600 text-sm">"${p.quote}"</p>
      </div>` : ''}

      <a href="${waGeneralLink}" target="_blank" class="mt-2 w-full bg-white border-2 border-[#25D366] text-[#25D366] p-4 rounded-2xl flex items-center justify-center gap-3 hover:bg-green-50 transition-all active:scale-[0.98] font-semibold">
...
function renderOnboardingModal(app) {
  ...
  modal.className = 'fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/50 backdrop-blur-sm'
  modal.innerHTML = `
  <div class="bg-white rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl">
    <div class="w-14 h-14 bg-stone-100 rounded-full mx-auto mb-5 flex items-center justify-center">
      <i data-lucide="sparkles" class="w-7 h-7 text-stone-600"></i>
    </div>
    <p class="text-xs text-stone-400 uppercase tracking-widest font-semibold mb-2">Bienvenida a</p>
    <h2 class="font-serif text-2xl text-stone-800 mb-4">Hunza Care</h2>
    <p class="text-stone-500 text-sm leading-relaxed mb-6">
      ...
      <strong class="text-stone-700">La Dra. Macarena ya cargó tu ficha.</strong>
    </p>
    <button id="btn-onboarding-done" class="w-full bg-stone-800 text-white py-3.5 rounded-2xl font-semibold hover:bg-stone-700 transition">
      Entendido, ver mi ficha →
    </button>
    <p class="text-xs text-stone-300 mt-3">Este mensaje no volverá a aparecer</p>
  </div>`
...
function renderNoAccess(app) {
  ...
  <div class="min-h-screen flex items-center justify-center p-4 bg-stone-50">
    <div class="bg-white p-8 rounded-3xl shadow-sm border border-stone-100 max-w-sm w-full text-center">
      <i data-lucide="shield-x" class="w-10 h-10 text-stone-400 mx-auto mb-4"></i>
      <h2 class="font-serif text-xl text-stone-800 mb-2">Sin acceso</h2>
      <p class="text-stone-500 text-sm mb-6">Tu cuenta no está registrada en Hunza Care. Contactá a la clínica para que la Dra. Macarena te agregue.</p>
      <button id="btn-logout-noaccess" class="w-full py-3 bg-stone-100 text-stone-700 rounded-2xl text-sm font-medium hover:bg-stone-200 transition">
        Cerrar sesión
      </button>
    </div>
  </div>`
