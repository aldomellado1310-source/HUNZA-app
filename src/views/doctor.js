  ...
  <div class="min-h-screen bg-stone-50 pb-20">
    <header class="bg-white border-b border-stone-200 sticky top-0 z-20 shadow-sm">
      <div class="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <button id="btn-logout" class="text-stone-400 hover:text-stone-700 transition p-2 bg-stone-50 rounded-full">
    ...
    <main class="max-w-5xl mx-auto px-4 py-6 flex flex-col md:flex-row gap-6">
      <div class="w-full md:w-1/3 flex flex-col gap-6">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="bg-white p-5 rounded-3xl shadow-sm border ${alertas > 0 ? 'border-red-100 bg-red-50/20' : 'border-stone-100'}">
            ...
            <p class="text-3xl font-serif ${alertas > 0 ? 'text-red-600' : 'text-stone-400'}">${alertas}</p>
          </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-3xl shadow-sm border border-stone-100">
          <div class="flex flex-col gap-3 mb-5">
            ...
            <div class="flex gap-2 w-full">
              <select id="periodFilter" class="w-1/2 text-sm border border-stone-200 rounded-xl bg-stone-50 px-3 py-2 text-stone-700 font-medium focus:ring-2 focus:ring-stone-200 outline-none">
                ${['semanal','mensual','anual'].map(v => `<option value="${v}" ${chartFilters.period === v ? 'selected' : ''}>${v.charAt(0).toUpperCase() + v.slice(1)}</option>`).join('')}
              </select>
              <select id="monthFilter" class="w-1/2 text-sm border border-stone-200 rounded-xl bg-stone-50 px-3 py-2 text-stone-700 font-medium focus:ring-2 focus:ring-stone-200 outline-none ${chartFilters.period === 'anual' ? 'opacity-40 cursor-not-allowed' : ''}" ${chartFilters.period === 'anual' ? 'disabled' : ''}>
      ...
      <div class="w-full md:w-2/3">
        <div class="flex items-center justify-between mb-4 px-1">
          <h3 class="font-serif text-xl text-stone-800">Seguimiento por Semáforo</h3>
          <button id="btn-nueva-paciente" class="flex items-center gap-2 bg-stone-800 text-white text-sm px-4 py-2 rounded-xl hover:bg-stone-700 transition font-medium">
...
function getPatientCardHTML(p) {
  return `
  <div class="bg-white p-5 rounded-2xl shadow-sm border ${p.priority === 'red' ? 'border-red-200 shadow-red-100' : p.priority === 'yellow' ? 'border-amber-200' : 'border-stone-100'} flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
    <div class="flex-1">
      <div class="flex items-center gap-2 mb-1.5">
        <h4 class="font-semibold text-stone-800 text-lg">${p.name}</h4>
        <span class="text-[10px] font-semibold px-2 py-0.5 rounded bg-stone-100 text-stone-600 uppercase truncate max-w-[200px]">${p.procedure}</span>
      </div>
      <p class="text-sm text-stone-600 mb-1 flex items-center gap-1.5 font-medium">
        <i data-lucide="clock" class="w-4 h-4 text-stone-400"></i> ${p.stage}
      </p>
    ...
    <div class="flex gap-2 p-1 bg-stone-50 rounded-full border border-stone-200 w-fit">
      ${['green','yellow','red'].map(color => `
        <button id="priority-${p.id}-${color}" class="w-10 h-10 rounded-full transition-all flex items-center justify-center ${p.priority === color ? `${color === 'green' ? 'bg-emerald-500' : color === 'yellow' ? 'bg-amber-400' : 'bg-red-500'} shadow-md scale-110` : 'bg-white hover:bg-stone-200'}">
          <div class="w-3 h-3 rounded-full ${p.priority === color ? 'bg-white' : color === 'green' ? 'bg-emerald-400' : color === 'yellow' ? 'bg-amber-400' : 'bg-red-500'}"></div>
        </button>`).join('')}
    </div>
  </div>`
}
...
function getAddPatientFormHTML() {
  return `
  <div id="add-patient-form" class="bg-white border border-stone-200 rounded-2xl p-5 mb-4 shadow-sm">
    <h4 class="font-serif text-lg text-stone-800 mb-4">Nueva Paciente</h4>
    <div class="grid grid-cols-1 gap-3">
      <input id="f-name" type="text" placeholder="Nombre completo" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-stone-200" required>
      <input id="f-email" type="email" placeholder="Email Gmail (para login)" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-stone-200" required>
      <input id="f-procedure" type="text" placeholder="Procedimiento" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-stone-200" required>
      <input id="f-stage" type="text" placeholder="Etapa actual (ej: Día 1 - Inflamación)" class="w-full border border-stone-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-stone-200" required>
      <div class="grid grid-cols-2 gap-3">
        <select id="f-priority" class="border border-stone-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-stone-200 bg-white">
          <option value="green">🟢 Verde</option>
          <option value="yellow" selected>🟡 Amarillo</option>
          <option value="red">🔴 Rojo</option>
        </select>
        <input id="f-nextControl" type="date" class="border border-stone-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-stone-200">
      </div>
    </div>
    <p id="form-error" class="text-red-500 text-xs mt-2 hidden"></p>
    <div class="grid grid-cols-2 gap-3 mt-4">
      <button id="btn-cancel-form" class="py-2.5 rounded-xl bg-stone-100 text-stone-600 text-sm font-medium hover:bg-stone-200 transition">Cancelar</button>
      <button id="btn-save-patient" class="py-2.5 rounded-xl bg-stone-800 text-white text-sm font-semibold hover:bg-stone-700 transition">Guardar →</button>
    </div>
  </div>`
}
